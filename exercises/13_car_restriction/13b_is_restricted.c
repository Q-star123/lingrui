...
enum day { MONDAY = 1, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY };//这里是为了方便用户输入

int is_restricted(int tail_num, enum day today) {//enum则是枚举，此处要注意，枚举只适用于这种不是数字不确定的类型
    int ret=0;
    switch(tail_num){
        case 0:
        case 5:
        ...
        default:
        ret =0;
        break;//这里一定要记住，case后面只能跟整数而且千万不要图省事吧条件写在一行
    }
    return ret;
}
